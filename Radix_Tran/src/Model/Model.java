...
public class Model {
     ...
     * 16进制字符串（空格区分)转换成字符串
     */
    private String getString_By16( String sSrc )
    {
    	int i = 0;
    	String sRet = null;
    	String sStr_16_To_10 = "";
    	String sStr_10_To_Str = "";
    	
    	String[] sourceStrArray = sSrc.split(" ");
    	int iLeng_Arrys = sourceStrArray.length;
    	for( i = 0 ; i < iLeng_Arrys; i ++ )
    	{    		
    		sStr_16_To_10 = Integer.valueOf( sourceStrArray[i],16).toString();
			char cChar_Tmp = (char)Integer.parseInt(sStr_16_To_10);
			sStr_10_To_Str = String.valueOf(cChar_Tmp);
			sRet = sRet + sStr_10_To_Str;
    	}
    ...
    public String getInfo(){
    	...
    	else
    	{
    		try {
		    	...
		    	else if ( -16 == iYjz && -2 == jMjz )
		    	{
		    		//16进制字符串（空格区分）转换成
		    		Logger.getGlobal().info("16进制字符串（空格区分）转成10进制byte");
		    		Msj = getString_By16(Ysj);
		    	}
    ...
    private int iGetjz(String str){
    	...
    	else if ( 0 == str.compareTo("16进制字符串-空格分割") )
    	{
    		Logger.getGlobal().info("16进制字符串-空格分割");
    		num = -16;
    	}
    	else if ( 0 == str.compareTo("字符串") )
    	{
    		Logger.getGlobal().info("字符串");
    		num = -2;
    	}
    	else
    	{
